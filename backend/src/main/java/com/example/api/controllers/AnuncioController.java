...
public class AnuncioController {
    ...
    @PostMapping("/criar")
    @Operation(summary = "Criar anúncio", description = "Cria anúncio e debita 50 créditos automaticamente")
    public ResponseEntity<?> criarAnuncio(@RequestParam("idImovel") Long idImovel) {
        try {
            ...
            return ResponseEntity.ok(Map.of(
                "success", true,
                "message", "Anúncio criado! 50 créditos debitados.",
                "anuncio", anuncio,
    ...
    @Operation(summary = "Publicar anúncio", description = "Publica anúncio pendente - define data expiração +30 dias")
    public ResponseEntity<?> publicarAnuncio(@PathVariable Long id) {
        try {
            Anuncio anuncioPublicado = anuncioService.publicarAnuncio(id);
            return ResponseEntity.ok(Map.of(
                "success", true,
                "message", "Anúncio publicado com sucesso",
                "status", anuncioPublicado.getStatusAnuncio(),
                "data_expiracao", anuncioPublicado.getDataExpiracao()
            ));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of(
                "success", false,
